...
Boom::Boom(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "Boom", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void Boom::OnParentWindowResize(int width, int height)
{
  if(GetUI())
...
#if IPLUG_DSP
void Boom::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{ 
