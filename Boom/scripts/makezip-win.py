...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\Boom Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\Boom Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\Boom manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\\Boom.vst3\\Contents\\x86_64-win\\Boom.vst3",
      projectpath + "\\build-win\\Boom_x64.exe"  
    ]
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Boom-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Boom-app_x64.pdb"  
  ]
